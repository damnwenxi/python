...
import random
import time
url = "https://www.xicidaili.com/nn/4"
...
#
#         proxy = {
#             tds[5].text.lower() : tds[1].text+':'+tds[2].text
#         }
#         ip_list2.append(proxy)
...
for i in range(len(ip_list)):
    try:
        ...
        vote1 = requests.post(url=vote_url, timeout=1,data={'id':vote_id}, headers = header_vote,proxies=proxy1).text
        vote2 = requests.post(url=vote_url, timeout=1,data={'id':vote_id}, headers = header_vote,proxies=proxy2).text
        print(vote1)
        print(vote2)
        if vote1 or vote2:
